...
int main(int argc, char **argv) {
    ...
    while (fgets(cmd, CMD_LEN, stdin) != NULL) {
        ...
        // Task 6: Wait for all background jobs
            ...
            if ((tokens.length-1)!=strvec_find(&tokens, "&")){
                ...
                if (pid == 0) {
                   ...
                   // return to shell
                    ...
                    // check child status, if stopped, add to jobs
                    if (WIFSTOPPED(status) != 0) {
                        if (job_list_add(&jobs, pid, strvec_get(&tokens, 0), JOB_STOPPED) == -1) {
                            fprintf(stderr, "job_list_add\n");
                            strvec_clear(&tokens);
                            job_list_free(&jobs);
                            return -1;
                        }
                ...
                if (pid == 0) {
                    if (run_command(&tokens) == -1) {
                        // if run into error, print error message
                        perror("run_command");
                        strvec_clear(&tokens);
                        job_list_free(&jobs);
                        return -1;
